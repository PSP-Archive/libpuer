...
int main(int argc, char **argv)
{
	...
	FILE* elf_file = fopen(argv[1], "r+b");
	
	if (elf_file)
	{
		int elf_size = 0;
		
		char* elf_buffer = (char*)malloc(10 * 1024 * 1024);
		elf_size = fread(elf_buffer, 1, 10 * 1024 * 1024, elf_file);
		
		Elf32_Ehdr* elf_hdr = (Elf32_Ehdr*)&elf_buffer[0];
		...
		{
			sec_hdr = (Elf32_Shdr*)&elf_buffer[elf_hdr->e_shoff + (i * sizeof(Elf32_Shdr))];
