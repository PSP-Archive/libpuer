...
#else
int Expand(char *pbp, char *dir, int argc, char **argv){
#endif
	int mode=MODE_DECRYPT, res;
	u8 pbp_header[0x28];
	SceUID fd;
	int error = 0;
...
_7gtable_size=_8gtable_size=_9gtable_size=
_10gtable_size=_11gtable_size=_12gtable_size=0;

#ifdef REV1
	if (ReadFile(pbp, 0, pbp_header, sizeof(pbp_header)) != sizeof(pbp_header)){
		if((KERNELVERSION&0xff0000)>0x040000&&!strcmp(pbp,"ms0:/401.PBP")){
			SceUID fd = sceIoOpen("ms0:/seplugins/popsloader/impose.prx", PSP_O_RDONLY, 0);
			if(fd<0)ErrorExit(5000, "Cannot find ms0:/401.PBP, whose impose.prx is required for popsloader.\n");
			sceIoClose(fd);
		}
	...
	if (ReadFile(pbp, 0, pbp_header, sizeof(pbp_header)) != sizeof(pbp_header)){
		if((KERNELVERSION&0xff0000)>0x040000&&!strcmp(pbp,"ms0:/401.PBP")){
			SceUID fd = sceIoOpen("ms0:/seplugins/popsloader/modules/impose.prx", PSP_O_RDONLY, 0);
			if(fd<0)ErrorExit(5000, "Cannot find ms0:/401.PBP, whose impose.prx is required for popsloader.\n");
			sceIoClose(fd);
		}
		printf("Cannot find %s.\n",pbp);return 1;
